...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=173',{updateViaCache:'none'}).then(r=>r.update()).catch(console.warn));
}

/* v17.3: symptom-based viewport recovery.
   No OS/browser version sniffing. The compatibility layout is enabled only
   when the app shell is actually outside the visible viewport. */
(function(){
  ...
  function shellIsAbnormal(){
    if(!header || document.body.classList.contains('auth-locked')) return false;
    const v=viewport(), r=header.getBoundingClientRect();
    if(Math.abs(v.scale-1)>0.03) return false; // do not fight intentional pinch zoom
    const xBad=r.left < v.left-3 || r.left > v.left+3 || r.right > v.left+v.width+3;
    const yBad=r.top < v.top-3 || r.bottom <= v.top+8;
    const scrollBad=Math.abs(window.scrollX)>2 || Math.abs(window.scrollY)>2;
    return xBad || yBad || scrollBad;
  }

  function applyRepair(){
    const v=viewport();
    repair=true;
    root.classList.add('viewport-repair');
    root.style.setProperty('--app-vv-left',v.left+'px');
    root.style.setProperty('--app-vv-top',v.top+'px');
    root.style.setProperty('--app-vv-width',v.width+'px');
    root.style.setProperty('--app-vv-height',v.height+'px');
  ...
  function check(){
    ...
    if(shellIsAbnormal()) applyRepair();
    else requestAnimationFrame(()=>{try{map.invalidateSize({pan:false})}catch(e){}});
  }

  function settle(){
    clearTimeout(timer);
    check();
    [80,220,500,900].forEach(ms=>setTimeout(check,ms));
  }

  window.addEventListener('load',settle,{passive:true});
  window.addEventListener('pageshow',settle,{passive:true});
  window.addEventListener('resize',settle,{passive:true});
  window.addEventListener('orientationchange',()=>setTimeout(settle,250),{passive:true});
